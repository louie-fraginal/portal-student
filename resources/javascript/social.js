...
async function initSocial() {
    ...
    const user = JSON.parse(localStorage.getItem('user')) || { firstName: profileName };
    const userInitial = document.getElementById('user-initial');
    const userFirstName = document.getElementById('user-firstname');
    
    if (userInitial) userInitial.textContent = user.firstName.charAt(0);
    ...
    const { data, error } = await window.supabaseClient
        ...
        data.forEach(post=> {
            ...
            const images = [post.image_2, post.image_3, post.image_4, post.image_5];
            ...
            const timeAgo = formatTimeAgo(new Date(post.date));
            ...
            postCard.addEventListener('click', function(e) {
                ...
                overlay.innerHTML = `
                    ...
                    <div class="post-modal-card">
                        ...
                        ${post.image_1 ? `<img src="${post.image_1}" class="modal-image" onclick="openFullImage('${post.image_1}')" style="cursor:pointer;"> ` : ''}
                        ...
                        <p class="modal-date">${Date(post.date)}</p>
                    </div>
                `;
...
function renderSocialFeed(notices) {
    ...
    notices.forEach(notice => {
        ...
        const timeAgo = formatTimeAgo(new Date(notice.created_at));
